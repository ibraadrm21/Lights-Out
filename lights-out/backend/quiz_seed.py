...
def seed_questions():
    ...
    # Check if we already have questions to avoid duplicates on restart if desired, 
    # but for seed script usually we might want to clear or just append. 
    # User didn't specify, but let's check count.
    cur.execute("SELECT COUNT(*) as c FROM questions")
    count = cur.fetchone()["c"]
    if count > 0:
        print(f"Database already has {count} questions. Skipping seed to avoid duplicates.")
        conn.close()
        return
